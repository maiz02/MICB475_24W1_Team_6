...
upf_high_asthma_ASVs <- core_members(upf_high_asthma, detection=0, prevalence = 0.7)
upf_high_noasthma_ASVs <- core_members(upf_high_noasthma, detection=0, prevalence = 0.7)

# What are these ASVs? you can code it in two different ways to see the same things
tax_table(prune_taxa(upf_high_asthma_ASVs,upf_phyloseq_final_high))
tax_table(prune_taxa(upf_high_noasthma_ASVs,upf_phyloseq_final_high))

# can plot those ASVs' relative abundance
prune_taxa(upf_high_asthma_ASVs,upf_high_RA) %>% 
  plot_bar(fill="Genus") + 
...
upf_high_asthma_list <- core_members(upf_high_asthma, detection=0.001, prevalence = 0.10)
upf_high_noasthma_list <- core_members(upf_high_noasthma, detection=0.001, prevalence = 0.10)
...
high_venn <- ggVennDiagram(x = upf_high_asthma_list_full, label_alpha = 1.0,
  label_size = 5,        
